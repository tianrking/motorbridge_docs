...
|-----------|-------------|-----------------|
| Git release tag | `motorbridge` repository | `v0.2.8` |
| Python package | `bindings/python/pyproject.toml` | `0.2.8` |
| Rust workspace crates | `Cargo.toml` `[workspace.package]` | `0.2.8` |
...
|----------|---------|-----------|
| Stable | `0.2.8` | Production ready |
| Pre-release | `0.2.8a1` | Testing only |
| Development | `0.2.8.dev1` | Internal use |
...
pip install -i https://test.pypi.org/simple/ motorbridge==0.2.8a1
```
...
```bash
# Install from Git
pip install git+https://github.com/tianrking/motorbridge@v0.2.8
...
# Upgrade to latest stable: 0.2.8
pip install --upgrade motorbridge
...
pip install -i https://test.pypi.org/simple/ motorbridge==0.2.8b1
...
6. **Verify installation:**
   ```bash
   pip install -i https://test.pypi.org/simple/ motorbridge==0.2.8
   python3 -m pytest tests/
   ```
...
twine upload dist/motorbridge-0.2.8.tar.gz
