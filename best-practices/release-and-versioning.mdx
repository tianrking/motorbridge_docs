...
|-----------|-------------|-----------------|
| Git release tag | `motorbridge` repository | `v0.2.7` |
| Python package | `bindings/python/pyproject.toml` | `0.2.7` |
...
|----------|---------|-----------|
| Stable | `0.2.7` | Production ready |
| Pre-release | `0.2.7a1` | Testing only |
| Development | `0.2.7.dev1` | Internal use |
...
pip install -i https://test.pypi.org/simple/ motorbridge==0.2.7a1
```
...
```bash
# Install from Git
pip install git+https://github.com/tianrking/motorbridge@v0.2.7
...
# Upgrade to latest stable: 0.2.7
pip install --upgrade motorbridge
...
pip install -i https://test.pypi.org/simple/ motorbridge==0.2.7b1
...
6. **Verify installation:**
   ```bash
   pip install -i https://test.pypi.org/simple/ motorbridge==0.2.7
   python3 -m pytest tests/
   ```
...
twine upload dist/motorbridge-0.2.7.tar.gz
