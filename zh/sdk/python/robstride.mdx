...
- 统一默认反馈 ID：`0xFD`；`feedback_id` / `host_id` 是上位机侧 ID，不是电机 `device_id`。
- 统一模式与原厂语义映射：
...
- `feedback-ids` 是 host_id 列表，范围 `0..255`，不是电机 ID。
- Python CLI 扫描会通过指定 host_id 的 ABI helper 精确探测每个 `feedback-ids`；非法 host_id 会直接报错，不会静默回退。

默认值：
- 默认完整列表：`feedback-ids=0xFD,0xFF,0xFE,0x00,0xAA`。
