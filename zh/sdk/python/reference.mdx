...
RobStride 默认反馈 ID 为 `0xFD`。`feedback_id` / `host_id` 是上位机侧地址，不是电机 `device_id`。
...
RobStride `motor_id` / `device_id` 会校验为 `1..255`；`feedback_id` / `host_id` 会校验为 `0..255`。CLI 扫描会通过指定 host_id 的 ABI helper 精确探测每个 `--feedback-ids`；非法 host_id 会直接报错，不会静默回退。
