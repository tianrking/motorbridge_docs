...
<Note>Source: `motor_cli/ROBSTRIDE_API.zh-CN.md`</Note>
...
这些型号的高层控制调用形态保持一致，但原生功能码参数不是同一张表。`read-param` 和 `write-param` 会根据 `--model` 选择对应型号的参数名称/类型表，再做 payload 编码或解码。实际使用 RS01-RS06 时，不要把 `rs-00` 当作通用占位。
