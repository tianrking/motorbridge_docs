...
> v0.4.0 状态：浏览器/WebSocket 场景推荐优先使用 `ws_gateway`。本地调试保持 `127.0.0.1` 绑定；若绑定到非回环地址，请设置 `MOTORBRIDGE_WS_TOKEN` 并让客户端在握手中携带 token。Damiao `dm-serial` 整臂扫描和多关节状态同步已对齐 `damiao_state_many`。
