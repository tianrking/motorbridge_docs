...
<Note>
`motorbridge-cli` 当前没有 `--version` 参数。可用 `python3 -c "import importlib.metadata; print(importlib.metadata.version('motorbridge'))"` 查看已安装 Python 包版本。
</Note>
...
| `--zero-range` | `neg-pi-pi` | RobStride 置零坐标范围：`neg-pi-pi` 写 `zero_sta=1`；`zero-2pi` 写 `zero_sta=0` |
| `--store` | `1` | 写入/置零后持久化保存 |
...
- `zero` / `set-zero`：当前仅 RobStride 支持；必须传 `--zero-exp 1` 才会下发置零序列。用同一个命令的 `--zero-range neg-pi-pi` 或 `--zero-range zero-2pi` 选择坐标范围，不需要手写 `0x7029` 参数。
...
| `--model` | 真实型号 | 真实 RobStride 型号（`rs-00` 到 `rs-06`）；参数表按型号区分，必须指定 |
| `--motor-id` | `127` | 电机 ID |
