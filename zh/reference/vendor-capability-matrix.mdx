...
| 厂商 | 原生能力入口（示例） | 说明 |
|------|----------------------|------|
| Damiao | `id-dump`, `id-set`, `damiao-read-param`, `damiao-write-param`（CLI）+ `set_zero_position()`（API） | 寄存器/参数读写、ID 修改、置零流程 |
| RobStride | `ping`, `robstride-read-param`, `robstride-write-param`, `run --mode read-param/write-param/save` | 29-bit 扩展帧参数通道；运行参数使用第 4 章通用表（`0x7005..0x702E`） |
