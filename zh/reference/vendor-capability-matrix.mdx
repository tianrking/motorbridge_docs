...
| RobStride | `ping`, `robstride-read-param`, `robstride-write-param` | 29-bit 扩展帧参数通道；参数表由 `model` 选择 |
...
`--model rs-00` 只适用于 RS00 电机。RS01-RS06 必须替换成真实型号，因为 `0x2000`/`0x3000` 手册参数表不同。
