...
MotorBridge v0.2.9 使用一张 RobStride 通用运行参数表，对齐协议第 4 章。
...
- `W/R` 或 `W` 只表示 CAN 上可写，不代表自动掉电保存。参数值需要掉电保持时，使用 `motorbridge-cli run --vendor robstride ... --mode save`。`--store 1` 用于置零流程。
