...
- **Damiao** 指令与寄存器调参：[damiao.mdx](damiao.mdx)
- **MyActuator** 命令与模式参考：[myactuator.mdx](myactuator.mdx)
- **RobStride** 指令与参数参考：[robstride.mdx](robstride.mdx)
...
说明：
- RobStride 默认 `--feedback-id` 为 `0xFD`；扫描默认尝试 host-id 候选 `0xFD,0xFF,0xFE,0x00,0xAA`。
...
详细的寄存器调参与模式参数请参见 [damiao.mdx](damiao.mdx)。
...
详细的参数 ID、协议覆盖与 WS 网关示例请参见 [robstride.mdx](robstride.mdx)。
...
详细的编码细节、反馈解码与故障排查请参见 [myactuator.mdx](myactuator.mdx)。
