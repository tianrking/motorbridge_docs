...
| Argument | Type | Default | Notes |
|---|---|---|---|
| `--set-motor-id` | u16 opt | none | New device ID, 1..255 |
...
- `--set-motor-id` / Python CLI `--new-motor-id` is validated as `1..255`; out-of-range values are rejected instead of being truncated.
