...
|--------|--------------------------------|-------|
| Damiao | `id-dump`, `id-set`, `damiao-read-param`, `damiao-write-param` (CLI) + `set_zero_position()` (API) | Register/parameter R/W, ID update, zeroing flow |
| RobStride | `ping`, `robstride-read-param`, `robstride-write-param`, `run --mode read-param/write-param/save` | 29-bit extended-frame parameter path; runtime parameters use the common section 4 table (`0x7005..0x702E`) |
