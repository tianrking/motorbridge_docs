...
Use `--model rs-06` only for a physical RS06 motor. For other RS00-RS05 motors, replace it with the actual model because the `0x2000`/`0x3000` manual parameter tables are different.
...
with Controller("can0") as ctrl:
    motor = ctrl.add_robstride_motor(6, 0xFD, "rs-06")  # replace with the physical RS00-RS06 model
    ctrl.enable_all()
