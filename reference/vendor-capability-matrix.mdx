...
- Always pass the physical RobStride model (`rs-00` ... `rs-06`). High-level control shape is shared, but manual function-code parameters differ by model.
...
| RobStride | `ping`, `robstride-read-param`, `robstride-write-param` | 29-bit extended-frame parameter path; parameter tables are selected by `model` |
...
Use `--model rs-00` only for an RS00 motor. For RS01-RS06, replace it with the actual model because the `0x2000`/`0x3000` manual parameter tables are different.
...
with Controller("can0") as ctrl:
    motor = ctrl.add_robstride_motor(6, 0xFD, "rs-00")  # replace with the physical RS00-RS06 model
    ctrl.enable_all()
