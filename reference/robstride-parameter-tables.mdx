...
- `W/R` or `W` only means the parameter can be written over CAN; it does not guarantee persistence by itself. Send type 22 save, or use CLI `--store 1`, when the value must survive power cycling.
