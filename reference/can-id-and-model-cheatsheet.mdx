...
RobStride model selection is required. RS00-RS06 use different manual function-code/parameter tables, so parameter read/write is encoded and decoded from the selected model. Do not use `rs-00` as a generic placeholder for other RS motors.
...
with Controller("can0") as ctrl:
    motor = ctrl.add_robstride_motor(
        ...
        model="rs-00"       # Replace with the physical RS00-RS06 model
    )
