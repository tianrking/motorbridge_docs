...
### Background Polling (current v0.2.7)

In current v0.2.7, the SDK uses background polling by default. You typically don't need to call `poll_feedback_once()`.

```python
# Recommended pattern (current v0.2.7)
motor.request_feedback()
state = motor.get_state()
```
