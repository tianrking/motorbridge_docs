...
For channel compatibility details (PCAN, slcan, CAN-FD, Damiao Serial Bridge), see [channel-compat.md](../../shared/channel-compat.md).
...
Default feedback ID for RobStride is `0xFD`. `feedback_id` / `host_id` is host-side addressing, not the motor `device_id`.
...
RobStride `motor_id` / `device_id` is validated as `1..255`; `feedback_id` / `host_id` is validated as `0..255`. CLI scan probes each listed `--feedback-ids` host_id exactly through host-id-specific ABI helpers, so invalid host IDs are rejected instead of silently falling back.
