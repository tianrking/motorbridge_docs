...
<Note>Source: `examples/README.md`</Note>
...
For RobStride examples, `rs-06` is only a sample model. Replace it with the physical RS00-RS06 model before real interaction. High-level control commands keep the same shape, but parameter read/write is selected from the model-specific RobStride table.
