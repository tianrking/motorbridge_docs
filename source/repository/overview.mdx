...
- RobStride:
  ...
  - model selection is required for real hardware use: parameter read/write uses common RobStride section 4 runtime tables for RS00-RS06, so do not reuse `rs-00` as a generic placeholder for other models
...
For RobStride, replace `--model rs-06` with the actual motor model (`rs-00` through `rs-06`). The model chooses the supported parameter table behind `read-param` / `write-param`.
