...
<Note>Source: `motor_cli/README.md`</Note>
...
- RobStride `--model` must match the physical motor for parameter access. RS00-RS06 use separate function-code tables, so `read-param` and `write-param` are typed from the selected model.
