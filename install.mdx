...
```bash
python3 -m pip install motorbridge==0.2.9
```
...
Expected output:
```
usage: motorbridge-cli [-h] {run,id-dump,id-set,scan,robstride-read-param,robstride-write-param,damiao-read-param,damiao-write-param} ...
...
python3 -m pip install -i https://test.pypi.org/simple/ motorbridge==0.2.9a1
```
