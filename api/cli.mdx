...
| `--zero-range` | `neg-pi-pi` | RobStride zero coordinate range: `neg-pi-pi` writes `zero_sta=1`; `zero-2pi` writes `zero_sta=0` |
...
- `zero` / `set-zero`: currently RobStride-only; must pass `--zero-exp 1` to actually send zero command sequence. Use `--zero-range neg-pi-pi` or `--zero-range zero-2pi` on the same command instead of writing `0x7029` manually.
